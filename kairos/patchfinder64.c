...
static ssize_t win_pread(int fd, void* buf, size_t count, addr_t offset) {
    long unsigned int ret = 0;
    OVERLAPPED lpOverlapped;
    bzero(lpOverlapped,sizeof(OVERLAPPED));
    lpOverlapped.Offset = (uint32_t)(offset & 0xFFFFFFFFLL);
    lpOverlapped.OffsetHigh = (uint32_t)((offset & 0xFFFFFFFF00000000LL) >> 32);
    HANDLE fp = (HANDLE)_get_osfhandle(fd);
    ReadFile(fp, buf, count, &ret, &lpOverlapped);
    return ret;
}
...
addr_t find_sandbox_handler(const char *name)
{
    ...
    struct handler {
        ...
        uint64_t func;
        uint64_t unk;
    } *handler = (struct handler*)(kernel + handler_map); // this as well?

    addr_t func = 0;
    while (handler->name && handler->func) {
        const char *hname = (char*)remove_pac((addr_t)handler->name) - (char*)kerndumpbase + (char*)kernel; // i guess this works? doesn't seem right
